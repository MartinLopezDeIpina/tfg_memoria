...
El sistema se basa en la generación dinámica de planes de acción, donde el agente principal elabora una estrategia inicial mediante un proceso de Cadena de Pensamiento \ref{plani}. Esta metodología permite desglosar problemas complejos en pasos intermedios de razonamiento. Posteriormente, un módulo procesador evalúa estos planes y determina si requieren una ejecución recursiva, descomponiéndolos en sub-tareas más específicas que se procesan en paralelo por agentes de la misma tipología.
...
Para este trabajo utilizaremos fundamentalmente LangChain para la gestión de llamadas a APIs de modelos y prompting, LangGraph para la orquestación de flujos agénticos, y LangSmith para el seguimiento de trazas de llamadas a modelos y herramientas. 

\section{Model Context Protocol}
todo: referencias a las docs.

El Model Context Protocol (MCP), desarrollado por Anthropic, estandariza la comunicación entre agentes LLM y herramientas. Permite que aplicaciones diversas ofrezcan herramientas a agentes externos sin exponer detalles de implementación.

La figura \ref{fig:mcp} representa el esquema operativo del protocolo. Como se puede observar, los desarrolladores de Jira y GitHub han implementado un servidor MCP encargado de proporcionar el conjunto de herramientas disponibles al cliente MCP. Dicho servidor efectúa la traducción de las interacciones necesarias con las APIs de estas aplicaciones, lo que permite que el desarrollador del agente se centre exclusivamente en la conexión de las herramientas del cliente con el agente. Paralelamente, el cliente MCP gestiona la comunicación entre los diferentes servidores, posibilitando que el agente acceda de forma directa a las diversas herramientas.
...
\begin{itemize}
  \item {\textbf{Sugerencias de autocompletado:}} Github Copilot o CursorAI ofrecen la integración de un modelo LLM ajustado para generación de código directamente al entorno del desarrollador. De esta forma, el desarrollador recibe sugerencias de autocompletado en tiempo real mientras escribe el código.  
